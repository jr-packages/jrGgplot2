...
\begin{itemize}
\item changing the transparency level: \cc{alpha};\marginnote{\cc{alpha} takes a value between $0$ and $1$.}
...
do?
\item What does \cc{stat\_bin2d()} and \cc{stat\_binhex()} do - add it to the
...
Hint: try \cc{stat\_smooth} and \cc{ylab(`New label')}.
...
\end{marginfigure}
\item Figure \ref{F2}: Using \cc{stat\_smooth}, add a loess line conditional
  on the drive.
<<echo=10*echo>>=
<<F1>>
@
