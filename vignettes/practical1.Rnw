...
Installing the the course R package\sidenote{A package is an \textit{add-on} or a \textit{module}. It provides additional functions and data.}
...
\newthought{To begin with}, load the \cc{ggplot2} package\marginnote{The \cc{ggplot2} package is
automatically installed with \cc{jrGgplot2}.}
