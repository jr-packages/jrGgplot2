...
To get you familiar with the underlying \textbf{ggplot2} concepts, we'll recreate
some standard graphics. Some these of plots aren't particularly useful, we are
just using them for illustration purposes.

\newthought{To begin with}, load the \textbf{ggplot2} package\marginnote{The \cc{ggplot2} package is
automatically installed with \textbf{jrGgplot2}.}
...
Some aesthetics, like \cc{shape} must be discrete. So we have to transform the variable into a
character or factor - \mbox{\cc{shape = factor(tenured)}}.
...
\textbf{default} adjustment is \cc{stack}
...
g + geom_bar(aes(x = gender, fill = dec),
             position = "dodge")
@
\caption{Barplot of ages using \cc{position = "dodge"}.}
\end{marginfigure}
\end{itemize}
